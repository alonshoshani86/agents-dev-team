...
def split_runs(content: str) -> List[dict]:
    ...
    # parts without matches: [content]
    # parts with N delimiters: [content_before, run1, agent1, ts1, content1, run2, ...]
    if len(parts) == 1:
    ...
    runs.append({"run": 1, "agent": "unknown", "timestamp": "", "content": first_content})

    # Remaining parts come in groups of 4: (run_num, agent, timestamp, content)
    idx = 1
    while idx + 3 < len(parts):
        run_num = int(parts[idx])
        agent = parts[idx + 1]
        timestamp = parts[idx + 2]
        run_content = parts[idx + 3]
        runs.append({"run": run_num, "agent": agent, "timestamp": timestamp, "content": run_content})
        idx += 4
...
def update_artifact_content(project_id: str, task_id: str, artifact_type: str, content: str) -> Optional[dict]:
    ...
    Treated as a new run appended with agent:human delimiter.
    """
    artifacts_dir = storage.project_tasks_dir(project_id) / task_id / "artifacts"
    content_path = artifacts_dir / f"{artifact_type}.md"
